...
##this was to view the first time
  ...
  mutate(keep_2020 = dplyr::case_when(id_pscis %like% 'Michel24' |
         ...
         notes_2020 = dplyr::case_when(id_pscis %ilike% 'Michel24' ~ 'same as MICH016. Completed in Masse 2015',
                                       ...
                                       id_pscis %ilike% 'Michel27' |
                                         id_pscis %like%'Michel51'
                                         ~ 'Roberts Ck non-fish in model due to steep gradients at mouth - could be hwy fill though',
                                       id_pscis %ilike% 'Morrisey06' ~ 'same crossing as MORR019 - assessed already',
                                       T ~ NA_character_))
